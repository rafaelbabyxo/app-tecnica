...
export function EditInformationForm({
  information,
  children,
  ...
  async function handleUpdateInfo(data: FormData) {
    if (name.trim() === '' || description.trim() === '') {
      toast({
        ...
        description:
          'Todas as informações são obrigatórias, você não pode atualizar uma informação com os campos vazios',
        variant: 'destructive',
      })
      return
    }
    ...
    if (
      ...
      console.log(
        '🚀 ~ file: index.tsx:46 ~ handleUpdateInfo ~ date === information.date:',
        date === information.date,
        { date, old: information.date },
      )
      toast({
        title: 'Informação não alterada!',
        description:
          'Antes de clicar no botão de Editar, por favor, altere alguma informação',
        variant: 'destructive',
      })
      return
    }
  ...
  return (
    ...
    >
      ...
      <textarea
        ...
        className="h-[142px] rounded-lg border border-[#C6C6C6] bg-white px-2 py-[0.375rem] text-black outline-none"
      />
